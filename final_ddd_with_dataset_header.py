...
# --- STATIC COOKIE CONFIGURATION ---
STATIC_JSESSIONID = "AF62FD4D9078A14D0A3652D44AF60B24"
...
HEADERS = {
    ...
    "Cookie": f"JSESSIONID={STATIC_JSESSIONID}" # <-- Added the static cookie here
}
...
def new_session():
    s = requests.Session()
    # It's also safe to inject the cookie directly into the requests.Session object 
    # to ensure it persists properly across all redirects and domains within the session.
    s.cookies.set('JSESSIONID', STATIC_JSESSIONID)
...
def extract_voter_data(epNo: str, state_code: str, max_retries: int = 5):
    ...
    while attempt < max_retries:
        try:
            ...
            # Re-apply the static cookie here as well just to be safe when overriding headers
            headers = {
                **HEADERS,
                "User-Agent": random.choice(UA_LIST),
                "Cookie": f"JSESSIONID={STATIC_JSESSIONID}" 
            }
